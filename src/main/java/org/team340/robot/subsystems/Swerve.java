...
public final class Swerve extends GRRSubsystem {
    ...
    public Swerve() {
        ...
        Tunable.pidController("Swerve/Auto PID", autoPIDx);
        Tunable.pidController("Swerve/Auto PID", autoPIDy);
        Tunable.pidController("Swerve/Auto Angular PID", autoPIDangular);
    }
     ...
     * @param pose A supplier that returns the new blue origin relative pose of the robot.
     */
    ...
    public void choreoController(SwerveSample sample) {
        ...
        api.applySpeeds(
            new ChassisSpeeds(
                autoPIDx.calculate(api.state.pose.getX(), sample.x) + sample.vx,
                autoPIDy.calculate(api.state.pose.getY(), sample.y) + sample.vy,
                autoPIDangular.calculate(api.state.pose.getRotation().getRadians(), sample.heading) + sample.omega
            ),
            Perspective.kBlue,
            true,
            false
