...
public class SwerveAPILogger extends ClassSpecificLogger<SwerveAPI> {
    ...
    @Override
    public void update(EpilogueBackend backend, SwerveAPI swerveAPI) {
        logState(backend.getNested("state"), swerveAPI.state);

        var errorHandler = Epilogue.getConfig().errorHandler;
        swerveAPI.imu.log(backend.getNested("imu"), errorHandler);
        for (var m : swerveAPI.modules) {
    ...
    private void logState(EpilogueBackend backend, SwerveState state) {
        backend.log("twist", state.twist, Twist2d.struct);
