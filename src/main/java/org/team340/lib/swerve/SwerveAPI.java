...
public class SwerveAPI implements AutoCloseable {
     ...
     * @param visionMeasurements Vision measurements to apply to the pose estimator.
     */
    public void refresh(VisionMeasurement... visionMeasurements) {
    ...
    public void applySpeeds(ChassisSpeeds speeds, Perspective perspective, boolean discretize, boolean ratelimit) {
        ...
        if (Math.abs(speeds.omegaRadiansPerSecond) >= w_max) {
            ...
            ChassisSpeeds relative = Math2.copyInto(speeds, new ChassisSpeeds());
            perspective.toRobotSpeeds(relative, state.pose.getRotation());
        ...
        lastRobotAngle = state.pose.getRotation();
        perspective.toRobotSpeeds(speeds, lastRobotAngle);
