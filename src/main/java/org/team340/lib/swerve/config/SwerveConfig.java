...
public class SwerveConfig {
    ...
    private List<String> missing() {
        ...
        if (period <= 0.0) missing.add("Period");
        if (odometryPeriod <= 0.0) missing.add("Odometry Period");
        if (discretizationPeriod <= 0.0) missing.add("Discretization Period");
        if (defaultFramePeriod <= 0.0) missing.add("Default Frame Period");
        if (movePID == null) missing.add("Move PID");
        if (moveFF == null) missing.add("Move FF");
        if (turnPID == null) missing.add("Turn PID");
        if (velocity < 0.0) missing.add("Velocity");
        if (velDeadband < 0.0) missing.add("Velocity Deadband");
        if (slipAccel < 0.0) missing.add("Slip Acceleration");
        if (torqueAccel < 0.0) missing.add("Torque Acceleration");
        if (angularAccel < 0.0) missing.add("Angular Acceleration");
        if (driverVel < 0.0) missing.add("Driver Velocity");
        if (driverVelExp < 0.0) missing.add("Driver Velocity Exponential");
        if (driverVelDeadband < 0.0) missing.add("Driver Velocity Deadband");
        if (driverAngularVel < 0.0) missing.add("Driver Angular Velocity");
        if (driverAngularVelExp < 0.0) missing.add("Driver Angular Velocity Exponential");
        if (driverAngularVelDeadband < 0.0) missing.add("Driver Angular Velocity Deadband");
        if (voltage < 0.0) missing.add("Voltage");
        if (moveStatorLimit < 0.0) missing.add("Move Stator Current Limit");
        if (moveSupplyLimit < 0.0) missing.add("Move Supply Current Limit");
        if (turnStatorLimit < 0.0) missing.add("Turn Stator Current Limit");
        if (turnSupplyLimit < 0.0) missing.add("Turn Supply Current Limit");
        if (moveGearRatio < 0.0) missing.add("Move Gear Ratio");
        if (turnGearRatio < 0.0) missing.add("Turn Gear Ratio");
        if (wheelDiameter < 0.0) missing.add("Wheel Diameter");
