...
public class SwerveRatelimiter {
    ...
    public SwerveState calculate(ChassisSpeeds desiredSpeeds) {
        ...
        if (desireStop) {
            // - Find the last state's total velocity across the X and Y axis.
            Translation2d lastV = new Translation2d(lastState.chassisSpeeds.vxMetersPerSecond, lastState.chassisSpeeds.vyMetersPerSecond);
            ...
            for (int i = 0; i < moduleCount; ++i) {
                ...
                desiredModuleStates[i].angle = wasMoving ? lastV.getAngle() : lastState.moduleStates[i].angle;
        ...
        for (int i = 0; i < moduleCount; ++i) {
            ...
            modulesLastVx[i] = lastState.moduleStates[i].angle.getCos() * lastState.moduleStates[i].speedMetersPerSecond;
            modulesLastVy[i] = lastState.moduleStates[i].angle.getSin() * lastState.moduleStates[i].speedMetersPerSecond;
            modulesLastHeading[i] = lastState.moduleStates[i].angle;

            // If the last module velocity was in reverse:
            //     - Flip the heading.
            if (lastState.moduleStates[i].speedMetersPerSecond < 0.0) {
        ...
        if (
            ...
            !Math2.twist2dEpsilonEquals(
                new Twist2d(
                    lastState.chassisSpeeds.vxMetersPerSecond,
                    lastState.chassisSpeeds.vyMetersPerSecond,
                    lastState.chassisSpeeds.omegaRadiansPerSecond
                ),
                Math2.TWIST2D_0
            )
        ) {
        ...
        if (!desireStop) {
            ...
            for (int i = 0; i < moduleCount; ++i) {
                ...
                if (Math2.epsilonEquals(lastState.moduleStates[i].speedMetersPerSecond, 0.0)) {
                    ...
                    if (Math2.epsilonEquals(desiredModuleStates[i].speedMetersPerSecond, 0.0)) {
                        overrideHeading.set(i, Optional.of(lastState.moduleStates[i].angle));
                        continue;
                    }

                    // - Find the rotational delta between the last and desired module heading.
                    Rotation2d rotDelta = lastState.moduleStates[i].angle.times(-1.0).rotateBy(desiredModuleStates[i].angle);
                    ...
                    if (Math.abs(rotDelta.getRadians()) / maxHeadingDelta <= 1.0) {
                        ...
                        overrideHeading.set(
                            i,
                            Optional.of(
                                lastState.moduleStates[i].angle.rotateBy(
        ...
        ChassisSpeeds constrainedSpeeds = new ChassisSpeeds(
            lastState.chassisSpeeds.vxMetersPerSecond +
            (dvScalar * (desiredSpeeds.vxMetersPerSecond - lastState.chassisSpeeds.vxMetersPerSecond)),
            lastState.chassisSpeeds.vyMetersPerSecond +
            (dvScalar * (desiredSpeeds.vyMetersPerSecond - lastState.chassisSpeeds.vyMetersPerSecond)),
            lastState.chassisSpeeds.omegaRadiansPerSecond +
            (dvScalar * (desiredSpeeds.omegaRadiansPerSecond - lastState.chassisSpeeds.omegaRadiansPerSecond))
        );
        ...
        for (int i = 0; i < moduleCount; ++i) {
            ...
            Rotation2d deltaRotation = lastState.moduleStates[i].angle.times(-1.0).rotateBy(constrainedStates[i].angle);
        ...
        SwerveState newState = new SwerveState(constrainedSpeeds, constrainedStates);
        // - Set the last state.
