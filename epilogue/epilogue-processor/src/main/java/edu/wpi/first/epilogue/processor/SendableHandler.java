...
public class SendableHandler extends ElementHandler {
    ...
    protected SendableHandler(ProcessingEnvironment processingEnv) {
        super(processingEnv);
        m_sendableType = lookupTypeElement(processingEnv, "edu.wpi.first.util.sendable.Sendable").asType();
        m_commandType = lookupTypeElement(processingEnv, "edu.wpi.first.wpilibj2.command.Command").asType();
        m_subsystemType = lookupTypeElement(processingEnv, "edu.wpi.first.wpilibj2.command.SubsystemBase").asType();
    }
    ...
    public String logInvocation(Element element) {
        ...
        if (
            m_processingEnv.getTypeUtils().isAssignable(dataType, m_commandType) ||
        ...
        return "logSendable(dataLogger.getSubLogger(\"" + loggedName(element) + "\"), " + elementAccess(element) + ")";
    }
}
