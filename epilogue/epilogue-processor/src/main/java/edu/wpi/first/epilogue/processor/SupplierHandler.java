...
public class SupplierHandler extends ElementHandler {
    ...
    protected SupplierHandler(ProcessingEnvironment processingEnv) {
        ...
        m_booleanSupplier = processingEnv
            ...
            .asType();
        m_intSupplier = processingEnv.getElementUtils().getTypeElement("java.util.function.IntSupplier").asType();
        m_longSupplier = processingEnv.getElementUtils().getTypeElement("java.util.function.LongSupplier").asType();
        m_doubleSupplier = processingEnv.getElementUtils().getTypeElement("java.util.function.DoubleSupplier").asType();
    }

    @Override
    public boolean isLoggable(Element element) {
        return (
            m_processingEnv.getTypeUtils().isAssignable(dataType(element), m_booleanSupplier) ||
