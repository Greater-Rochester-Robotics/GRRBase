...
public class AnnotationProcessor extends AbstractProcessor {
    ...
    private static final String kClassSpecificLoggerFqn = "edu.wpi.first.epilogue.logging.ClassSpecificLogger";
            ...
            .filter(e ->
                processingEnv
                    ...
                    .isAssignable(
                        ...
                        processingEnv
                            .getTypeUtils()
                            .erasure(processingEnv.getElementUtils().getTypeElement(kClassSpecificLoggerFqn).asType())
                    )
            )
        ...
        for (ExecutableElement method : methods) {
            ...
            if (method.getAnnotation(NotLogged.class) == null) {
                ...
                if (!method.getModifiers().contains(Modifier.PUBLIC)) {
                    ...
                    processingEnv
                        .getMessager()
                        .printMessage(Diagnostic.Kind.ERROR, "[EPILOGUE] Logged methods must be public", method);
                ...
                if (method.getModifiers().contains(Modifier.STATIC)) {
                    processingEnv
                        .getMessager()
                        .printMessage(Diagnostic.Kind.ERROR, "[EPILOGUE] Logged methods cannot be static", method);
                ...
                if (method.getReturnType().getKind() == TypeKind.NONE) {
                    processingEnv
                        .getMessager()
                        .printMessage(Diagnostic.Kind.ERROR, "[EPILOGUE] Logged methods cannot be void", method);
    ...
    private Map<DeclaredType, DeclaredType> processCustomLoggers(
        ...
        for (Element annotatedElement : annotatedElements) {
            ...
            boolean hasPublicNoArgConstructor = annotatedElement
                .getEnclosedElements()
                .stream()
                .anyMatch(
                    enclosedElement ->
    ...
    private void warnOfNonLoggableElements(TypeElement clazz) {
        ...
        for (Element element : clazz.getEnclosedElements()) {
            ...
            if (
                element instanceof ExecutableElement exe &&
                exe.getModifiers().contains(Modifier.PUBLIC) &&
                exe.getParameters().isEmpty()
            ) {
                // isNotLoggable will internally print a warning message
