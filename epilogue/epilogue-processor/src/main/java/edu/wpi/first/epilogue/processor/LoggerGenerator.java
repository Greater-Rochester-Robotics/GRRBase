...
public class LoggerGenerator {
    ...
    public void writeLoggerFile(TypeElement clazz) throws IOException {
        ...
        Predicate<Element> optedIn = e -> !requireExplicitOptIn || e.getAnnotation(Logged.class) != null;

        var fieldsToLog = clazz
            .getEnclosedElements()
            .stream()
    ...
    private void writeLoggerFile(
        ...
        if (!classConfig.name().isBlank()) {
            loggerSimpleClassName = StringUtils.capitalize(classConfig.name().replaceAll(" ", "")) + "Logger";
            if (lastDot > 0) {
        ...
        try (var out = new PrintWriter(loggerFile.openWriter())) {
            ...
            out.println(
                "public class " + loggerSimpleClassName + " extends ClassSpecificLogger<" + simpleClassName + "> {"
            );
            ...
            var loggedElementsByImportance = Stream.concat(loggableFields.stream(), loggableMethods.stream()).collect(
                groupingBy(
                    element -> {
            ...
            loggedElementsByImportance.forEach((importance, elements) -> {
                ...
                for (var loggableElement : elements) {
                    ...
                    var handler = m_handlers.stream().filter(h -> h.isLoggable(loggableElement)).findFirst();
